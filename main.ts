...
export default class InlineCheckboxPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.registerDomEvent(document, "click", (evt) => this.onClick(evt as MouseEvent));
    this.addStyles();

    // Add CodeMirror extension for edit mode checkboxes
    // @ts-ignore
    if ((this.app as any).workspace?.on) {
      ...
      this.registerCodeMirror((cm: any) => {
        cm.on("renderLine", (cm: any, line: Element, elt: HTMLElement) => {
  ...
  private process(root: HTMLElement, ctx: MarkdownPostProcessorContext) {
    ...
    while ((node = walker.nextNode())) {
      ...
      if (parent.closest("li")?.textContent?.trim().startsWith("[") && parent.closest("ul,ol")) continue;
      ...
      parent.replaceChild(frag, node as Text);
    }
  }

  private async onClick(evt: MouseEvent) {
    const target = evt.target as HTMLElement;
    if (!(target instanceof HTMLInputElement) || !target.classList.contains("inline-cb")) return;
    ...
    if (!filePath || !secStartStr) return;
